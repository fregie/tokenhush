...
// time.Now, BinaryVersion to Version, LookupEnv to os.LookupEnv and Budget to
// filter.PrimitiveByteBudgetBytes.
type RulesSyncConfig struct {
	...
	// Budget is the per-primitive byte budget every compiled pack rule scans
	// with. Zero means filter.PrimitiveByteBudgetBytes, so a caller that does
	// not set it keeps the documented default.
	Budget int
}
...
func NewRulesSync(cfg RulesSyncConfig) (*RulesSync, error) {
	...
	budget := cfg.Budget
	if budget <= 0 {
		budget = filter.PrimitiveByteBudgetBytes
	}
	...
	return &RulesSync{
		...
		highWater: highWater, now: now, binary: parsed, lookupEnv: lookupEnv, budget: budget,
...
func (s *RulesSync) accept(manifest RulesManifestPayload, bundleRaw []byte, gateOpen bool) (acceptedPack, error) {
	...
	compiled, err := filter.CompileWithBudget(doc, s.budget)
	if err != nil {
		return acceptedPack{}, err
	}
