...
- **可逆脱敏** —— 请求 body 里检测到的每个密钥都变成会话级 `__PII_<type>_<digest>__` 占位符，响应回来时还原成原值。模型只看得到占位符。
- **6 个内置检测器** —— 默认开启 `prefix`、`jwt`、`pem`、`luhn`、`email`；`entropy` 需显式开启。
- **14 个 AI 编程工具** —— 每个都能用 `tokenhush env <tool>` 拿到可粘贴的 base URL 片段。
- **仅回环** —— 只绑定 `127.0.0.1`（有 IPv6 回环时加 `[::1]`）；不终止 TLS、不装根证书、不做 MITM。
- **纯 Go，`CGO_ENABLED=0`** —— 仅一个直接依赖，单文件二进制覆盖 macOS / Linux / Windows。
- **Apache-2.0** —— 默认无遥测；仅有的两类外发请求（更新检查、规则同步）都可各自关闭。
