...
- **Signed rule sync, with a non-weakening floor.** `tokenhush rules sync` fetches an Ed25519-signed rule pack and verifies its signature, freshness, serial (no rollback), and the signed revocation list before use. The floor rejects any pack that disables a built-in detector, drops a required category, or carries an `allow` action, so a pack can never weaken the built-ins. Packs load at the next start, never hot, and any problem falls back to the built-in defaults with a warning.
