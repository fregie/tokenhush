...
- 默认是 `FailOpenWarn`（失败开放）：出错、超时或 panic 时，核心丢弃该插件的 findings，通过注入的 `AuditSink` 接缝发出审计警告，并让请求继续。
- `FailClosed`（失败关闭）：对关键检测器，失败会拒绝请求（返回 `Block`）。内置检测器使用这一更严格的设置。
- 无论采用哪种策略，**失败总会通过接缝发出审计警告**。不会有任何东西被悄无声息地吞掉。核心默认 sink 是 no-op；私有 Pro 层负责持久化该警告。
