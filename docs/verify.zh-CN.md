...
没有证明：完美的召回率。检测器是确定性的、高精度优先（见 [security.zh-CN.md](security.zh-CN.md)）；任何检测器都不认识的取值会原样转发。本演示展示的硬性不变量是单向的：**占位符绝不向出站方向回填**（[security.zh-CN.md](security.zh-CN.md#硬性不变量)）。
