...
没有证明：完美的召回率。检测器是确定性的、高精度优先（见 [security.zh-CN.md](security.zh-CN.md)）；任何检测器都不认识的取值会原样转发。也没有证明已知密钥的每一种编码形态都会被拦下：脱敏之后的出站复核是一组固定的解码器枚举，其已知不覆盖类别发布在[已知限制](security.zh-CN.md#已知限制)一节。本演示展示的硬性不变量是单向的：**占位符绝不向出站方向回填**（[security.zh-CN.md](security.zh-CN.md#硬不变量)）。
