...
Tokenhush 是一个本地基础 URL 网关：你的 AI 编程工具将请求发送到 `http://127.0.0.1:8787`，网关在将请求转发到上游之前对密钥和敏感数据脱敏，然后记录一条本地审计时间线。一切都在你的机器上运行，公开核心从不安装根证书或执行 MITM。这些文档涵盖如何安装和运维它、它如何构建，以及如何扩展它。
...
| [architecture.zh-CN.md](architecture.zh-CN.md) | 请求路径、模块布局、审计链与开源核心边界 |
...
**运维者。** 阅读 [security.zh-CN.md](security.zh-CN.md) 了解不变量与威胁模型，然后阅读 [deployment.zh-CN.md](deployment.zh-CN.md) 第 5 节了解控制 token 和审计数据库的位置，第 10 节了解部署约束。
