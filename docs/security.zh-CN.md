...
两个域共用同一套检测器规则，包括 `high_entropy` 的**结构化标识符豁免**：服务商分配的不透明 id（`call_…`、`toolu_…`、`chatcmpl-…`、`msg_…`、`resp_…`）不被当作密钥。被豁免的语法在 `KnownStructuralIdentifierExemptions()` 中枚举（镜像于 `pkg/redact/testdata/known_structural_exemptions.txt`）；残余风险记录于下方「已知限制」。
...
15. **`high_entropy` 的结构化标识符豁免是跳过，不是判定。** 符合被豁免语法（`call_…`、`toolu_…`、`chatcmpl-…`、`msg_…`、`resp_…`）的密钥不会被脱敏。若引擎已知该密钥，出站复核仍会拒绝该请求（403、上游零字节；`StructuralIdentifierContains`）；该形态下引擎**未知**的密钥即记录在案的残余风险。语法在 `KnownStructuralIdentifierExemptions()` 中枚举。`high_entropy` 对纯 hex 的排除未变（第 12 条）。
