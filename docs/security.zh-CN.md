...
**守护做什么。** 变更通道检测运行在响应方向，针对模型发起的工具调用参数。模式集是显式、可枚举、可审计的（`pkg/proxy` 的 `MutationChannelPatternInventory()`）。控制端口类要求**控制通道证据，而非仅凭端口**：环回 host 加真实控制端口，**并且**带控制端点路径或控制请求行。同一端口上的数据面路径（例如 `http://127.0.0.1:8787/v1/chat/completions`）是合法流量，不会被拒绝。`/status` 是只读元数据（只报告计数），因此只有与**变更**同时出现才算证据——写动词（POST/PUT/PATCH/DELETE）或 curl 数据 flag；只读的 `GET`/`HEAD` `/status` 刻意不被拒绝。`/allowlist` 保持无条件：它是变更端点，读与写都仍是证据。提供证据的 HTTP 客户端或动词本身必须是其 shell 段的**命令词**（执行边界）：控制路径或请求动词若是另一条命令的参数——`grep` 模式、`echo` 字符串、提交消息——即为提及，而不是一次触及。
...
- **提及不等于调用，判定是位置性的（执行边界 + 命令词）。** 在行动型工具内部，受守护 token 只有**同时**满足以下条件才算一次调用：它位于 **shell 段起点**——候选文本开头，或紧接 `;`、`&&`、`||`、`|`、换行、反引号、`$(`，或 `sh -c "`/`bash -c '` 之类 shell 包装开启符之后；并且它是该段的**命令词**，即其前只有空白、包装命令（`sudo`、`doas`、`env`、`nohup`、`command`、`exec`、`eval`、`time`、`nice`、`setsid`、`stdbuf`、`timeout` 等）、包装 flag 与环境赋值（`FOO=bar`），路径前缀（`/usr/local/bin/tokenhush`、`C:\Program Files\Tokenhush\tokenhush.exe`）属于命令词的一部分；并且其后紧跟该类既有的子命令/flag 形状。**其余一切皆为提及，一律放行**：作为另一条命令参数的 token、位于正常（非代码）引号字符串内的 token、以及位于内容型 here-document 主体内的 token。因此 `grep -rn "<CLI 形式>" docs/`、`rg`、`sed`/`awk` 打印匹配行、`echo`/`printf` 引用它、`git log --grep='<CLI 形式>'`、`git commit -m` 消息点名它，以及裸散文提及，都在字面存在时放行；真正的调用——段起点的 `tokenhush allowlist …`、`sudo`/`env` 包装、路径前缀、`sh -c "…"`、反引号、`$()`、`;`/`|`/`&&` 之后——仍被拒绝。here-document 主体是内容，除非该 here-document 被喂给 shell 或解释器（那样保持检查）。控制端口类遵循同一原则：提供证据的 HTTP 客户端（`curl`/`wget`/`nc`）或请求动词本身必须是其段的命令词，控制路径或 URL 若作为另一条命令的参数（`grep` 模式、`echo` 字符串、提交消息）出现即为提及。冻结的 `MutationChannelMentionCarrierCommands()` 名单被保留为控制端口证据位置上的**额外**提及信号（那里 host:port 本就是 HTTP 客户端的参数）；对 CLI 类而言它已被命令词规则涵盖。
