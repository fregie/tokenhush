...
5. **回客户端响应在提交任何状态码之前解码。** identity 编码的
   ...
   最后执行，且只还原本会话铸造过的占位符。还原是**转义/深度感知**的：还原的
   secret 按外层 JSON 深度重新拼写（与客户端发出的拼写完全一致），因此多行或
   带引号的 secret 不会破坏客户端的 JSON。原始流片段或非 JSON body 保持原始
   拼写；外部占位符按字节原样返回。在**同一通道身份**、**连续**的
   **完整 JSON 信封** `data:` 事件之间做内容层拆分的占位符会被精确还原一次：
   token 未解析期间，受影响通道的贡献分段
   被扣留、不发送；一旦 token 解析（通常是接下来 1-2 个事件），每个被扣留分段
   就针对**自身原始字节**应用一次编辑，贡献片段被删除、由补全叶携带 secret，
   因此每个发出的信封都保持合法 JSON。联合 **256 KiB** 预算
   （`protocol.SSEBackfillHoldbackBytes`；写者尾部加上未解析 carry 再加上被扣留
   分段）为扣留内存设界；一旦将超限，被扣留分段按字面释放、不还原，流继续推进。
   新增延迟仅限于**受影响通道**，其余流量照常流动：对该通道而言，正确性
   （绝不发出不完整占位符）与有界内存优先于零新增延迟。**撕裂的内层 JSON
   起始叶**（非 `Encoded`、非法 JSON、以 `{` 或 `[` 开头）、其 secret 需要
   JSON 转义时，会保留**占位符**，而不是破坏客户端嵌套文档——即
   `docs/security.md` R6 记录的残余。若非匹配通道的事件在两半之间交织进来，
   被扣留的分段会按原样释放，该拆分只会以字面形式送达、不还原。
