...
- **有预算上限的原始检测器。** 原始类型规则（`prefix`、`email`、`luhn`、`jwt`、
  ...
  把该每叶、每检测器预算与 `scan_budget_bytes` 对齐，因此每个叶的检测成本保持
  O(budget)，预算也绝不是静默盲区：超过预算的请求叶会在 stderr 上报（仅元数据），
  且不移动任何计数器。
...
4. **出站请求先走叶遍历、求值与重写。** 声明为 JSON 却无法遍历的 body 以 400
   ...
   415 拒绝。随后 body 本身在共享读取 seam 处、以 `max_body_bytes`（默认
   64 MiB）为界一次性读取——它是内存护栏而非扫描预算：声明的 `Content-Length`
   超过上限时一个字节都不读就拒绝，否则读取经由 `http.MaxBytesReader`，cap+1
   字节会触发带类型的溢出。超过上限的 body 返回 403 `body_too_large`，绝不截断、
   绝不部分转发；恰好等于上限的 body 整段读取。请求阶段决策随后允许、阻断或替换。
   替换是**逐 span 精确**的：每个
   解码后的检测 span 都会映射回拼写它的原始转义字节，因此多行或带引号的 secret
   以占位符离开、并原样返回。脱敏日志行与 `redactions` 计数器只统计真正改变了
   字节的替换。原始类型检测器每个叶最多扫描 `scan_budget_bytes` 字节——每叶、
   每检测器预算与配置的扫描预算对齐——超过预算的叶会在 stderr 上报
