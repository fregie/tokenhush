...
Both domains share the detector rules, including `high_entropy`'s **structural exemptions**: provider-assigned opaque ids (`call_…`, `toolu_…`, `chatcmpl-…`, `msg_…`, `resp_…`), data-URI base64 payloads (`data:<mime>[;param];base64,…`), base64-alphabet payload runs of 128 bytes or more, and absolute paths carrying a hash segment of 32 or more hex characters are not treated as secrets. The exempted grammars are enumerated in `KnownStructuralIdentifierExemptions()` (mirrored in `pkg/redact/testdata/known_structural_exemptions.txt`); the residual risk is recorded under "Known limitations" below.
